...
bert_tokenizer = BertTokenizer.from_pretrained('tohoku-nlp/bert-base-japanese-whole-word-masking')
bert_model = BertModel.from_pretrained('tohoku-nlp/bert-base-japanese-whole-word-masking')
print('Done')
...
def main():
    ...
    # agent_utterance = r'C:\Users\hirok\OneDrive\Desktop\Greta_googletts\1.src\bin\Examples\DemoEN\reseponse\generate.xml'
    # agent_utterance = r'C:\Users\hirok\OneDrive\Desktop\Greta_ASAP\greta_ASAP_ASR_SST_20230710\greta_ASAP_ASR_SST_20230710\greta-gpl-grimaldi_ASAPnASR\bin\Examples\DemoEN\reseponse\generate.xml'
    agent_utterance = r'C:\Users\hirok\OneDrive\Desktop\Greta_ASAP\greta_ASAP_ASR_SST_20230710\greta_ASAP_ASR_SST_20230710\greta_ASAP_googleTTS\bin\Examples\DemoEN\reseponse\generate.xml'
    
    if not WITH_FEEDBACK:
        speak_greta('評価フィードバックは行わずにロールプレイのみを実施します\n', agent_utterance)
    ...
    if ASR_BACKGROUND:
        self_intro(m, agent_utterance, task_name, asr_client)
    else:
        self_intro(m, agent_utterance, task_name)
    ...
    while True:
        ...
        speak_greta(s5, agent_utterance)
        ...
        if ASR_BACKGROUND:
            ...
            subprocess.call("recognize.bat", shell=True, stdout=open(os.devnull, 'wb'))
    ...
    if WITH_FEEDBACK:
        ...
        speak_greta(utter, agent_utterance, block = True)
    
        print("#####################################")
        print("########## record_copy.bat ##########")
        print("#####################################")
        ...
        print("#####################################################")
        print("########## eval_pipeline\multi_pipeline.py ##########")
        print("#####################################################")
        ...
        speak_greta("ではフィードバックです。{}{}\n".format(feedback_pos, feedback_neg), agent_utterance, block = False)
        ...
        utter = "以上でロールプレイは終了です。ありがとうございました。評価フィードバックは行わずに終了します。 \n"
        speak_greta(utter, agent_utterance, block = True)
...
def self_intro(m, agent_utterance, task_name, asr_client = None):
    
    utter="こんにちは。レイと申します。お名前お伺いしてもよろしいですか？\n"
    speak_greta(utter, agent_utterance)
    ...
    elif ((task_name == "FAVOR") or (task_name == "ASK")):
        inst=json_load["FAVOR"]
        task_name_agent="頼みごとをするスキル"
    elif ((task_name == "REFUSE") or (task_name == "DECLINE")):
...
def write_xml(s5, agent_utterance):
    ...
    with open(agent_utterance, mode="w") as f:
        f.write(s1)
        f.write(s2)
        f.write(s3)
        f.write(s4)
...
def write_smile_xml(agent_utterance):
...
def speak_greta(utter, agent_utterance, block = True):
    
    print("Agent utterance: {}".format(utter))

    if ("(s)" in utter) == True:
        print("smile detected")
        #utter_pre=re.sub("\(.+?\)", "", utter) # delete all
        utter=utter.replace("(s)", "")
        write_xml(utter, agent_utterance)
        
        subprocess.call("dummy.bat", shell=True, stdout=open(os.devnull, 'wb'))
        subprocess.call("dummy.bat", shell=True, stdout=open(os.devnull, 'wb'))
        ...
        write_smile_xml(agent_utterance)
